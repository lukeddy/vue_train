    ...
    <div id="album" v-if="album">
      <header class="album-header">
          <div class="row">
              ...
              <div class="col-md-8">
                  <h4 v-if="album.artists&&album.artists.length > 0">
                    <span v-bind:key="mykey" v-for="(artist,mykey) in album.artists">
                      {{artist.name}}
                    </span>
                  </h4>
                  <h2>{{album.name}}</h2>
      ...
      <div v-if="album.tracks" class="album-tracks">
        <h2>专辑曲目</h2>
        <div class="col-md-3" v-bind:key="mykey" v-for="(track,mykey) in album.tracks.items">
            <div class="well">
